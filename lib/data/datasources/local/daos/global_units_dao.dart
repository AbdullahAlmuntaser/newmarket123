...
class GlobalUnitsDao extends DatabaseAccessor<AppDatabase> with _$GlobalUnitsDaoMixin {
  ...
  Future<int> addUnit(GlobalUnitsCompanion unit) => into(globalUnits).insert(unit);

  Future<bool> updateUnit(GlobalUnit unit) => update(globalUnits).replace(unit);

  Future<int> deleteUnit(String id) => (delete(globalUnits)..where((t) => t.id.equals(id))).go();
}
