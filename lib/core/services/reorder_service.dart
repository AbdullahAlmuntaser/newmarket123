...
class ReorderService {
  ...
  Future<List<String>> generateAutoPurchaseOrders({required String warehouseId}) async {
    ...
    for (var supplier in suppliers) {
      final itemsToOrder = lowStockProducts.where((p) => true).toList(); // Simplified logic
      ...
      await db.transaction(() async {
        ...
        for (var product in itemsToOrder) {
          await db.into(db.purchaseOrderItems).insert(PurchaseOrderItemsCompanion(
